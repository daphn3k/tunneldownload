...
def get_video_urls_from_session(session, cookie, perspective):
    ...
    preview_containers = soup.select_one('#main > div > div:nth-child(2) > div').find_all('div', class_='media_container_responsive', recursive=False)
    urls = []
    for container in preview_containers:
        container_perspective = container.select_one('div.media_container_responsive > div:nth-child(2) > span').text.strip()
...
def download_sessions(sessions):
    ...
    for session in sessions:
        logging.info('Downloading {} videos from session {}'.format(len(session['video_urls']), session['session_time']))
        date_path = os.path.join('media', session['session_time'].strftime('%Y-%m-%d'))
        session_path = os.path.join(date_path, session['session_time'].strftime('%H_%M'))
        ...
        for url in session['video_urls']:
            ...
            logging.info('Downloaded {} successfully to {}'.format(file_name, session_path))
...
def main():
    ...
    parser.add_argument('--perspective', help='Set perspective for downloading. Defaults to all perspectives.')
    parser.add_argument('--start', help='Set start date for downloading. Defaults: today-30days')
    ...
    for item in items:
        ...
        if session_time.date() >= start_date:
            valid_sessions.append({'filter_url': filter_link['href'], 'session_time': session_time, 'video_urls': []})
        # print(item.text)

    logging.info('Found {} valid sessions'.format(len(valid_sessions)))

    for session in valid_sessions:
        session['video_urls'] = get_video_urls_from_session(session, cookie, args.perspective)
        logging.info('Fetched {} video urls for session {}'.format(len(session['video_urls']), session['session_time']))
